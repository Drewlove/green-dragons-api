...
const service = {
  getAllRows(knex) {
    return knex.select("*").from(tableName).orderBy("last_name", "ASC");
  },
  getById(knex, row_id) {
    return knex
      .from(tableName)
  ...
  updateRow(knex, row_id, newFields) {
    return knex(tableName).where(`${tableName}_id`, row_id).update(newFields);
  },
  deleteRow(knex, row_id) {
    return knex(tableName).where(`${tableName}_id`, row_id).delete();
  },
};
