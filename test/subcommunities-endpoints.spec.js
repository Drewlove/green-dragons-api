...
const {makeParentTableRows, makeTestRows, makeTestRow, makeMaliciousRow} = require('./subcommunities.fixtures')
const logger = require('../src/logger')
const API_TOKEN = process.env.API_TOKEN

const table = {
  name: 'subcommunity', 
  parentTableName: 'community',
  endpoint: 'subcommunities',
  columns: ['subcommunity_name', 'community_id'],
  xssColumn: 'subcommunity_name',
  updatedColumn: {
    subcommunity_name: 'updated community name'
  },
  rowId: 'subcommunity_id'
}
...
describe(`${table.name} endpoints`, function() {
  ...
  before('make knex instance', () => {
    ...
    before('clean the parent table', () => db.raw(`TRUNCATE table ${table.parentTableName} RESTART IDENTITY CASCADE`))

    afterEach('cleanup',() => db.raw(`TRUNCATE table ${table.name} RESTART IDENTITY CASCADE`))
    afterEach('cleanup parent table',() => db.raw(`TRUNCATE table ${table.parentTableName} RESTART IDENTITY CASCADE`))
    ...
    context(`Given an XSS attack`, () => {
        ...
        it('removes XSS attack content', () => {
            ...
            .expect(200)
            .expect(res => {
                expect(res.body[0][table.xssColumn]).to.eql(expectedRow[table.xssColumn])
            })
        })
      })
    ...
    context(`Given an XSS attack row`, () => {
        ...
        it('removes XSS attack content', () => {
            ...
            .expect(200)
            .expect(res => {
                expect(res.body[table.xssColumn]).to.eql(expectedRow[table.xssColumn])
            })
        })
    })
                ...
                const expectedRow = {
                  ...
                  .then(res =>
                    supertest(app)
                      .get(`/api/${table.endpoint}/${idToUpdate}`)
                      .set('Authorization', `Bearer ${API_TOKEN}`)
                      .expect(expectedRow, logger.info(expectedRow))
                  )
              })
            })
          })
